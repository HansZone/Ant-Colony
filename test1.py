...
etatable = 1.0 / (distmat + np.diag([1e10] * numcity))  # 启发函数矩阵，表示蚂蚁从城市i转移到矩阵j的
...
while iter < itermax:
    ...
    for i in range(numant):
        ...
        for j in range(1, numcity):  # 循环numcity-1次，访问剩余的numcity-1个城市
            ...
            k_num = 0
            for num_cum in range(0, len(cumsumprobtrans)):
                if cumsumprobtrans[num_cum] > 0:
                    k_num = num_cum
                    break
            k = listunvisited[k_num]  # 下一个要访问的城市
            pathtable[i, j] = k
            unvisited.remove(k)
            # visited.add(k)
